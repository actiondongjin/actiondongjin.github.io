...
- 데이터 세트에서 중복을 허용하여 데이터를 샘플링하는 방식인 부트스트랩 샘플을 사용하고 랜덤하게 일부 특성을 선택하여 트리를 만듦
- RandomForestClassifier : 랜덤포레스트 분류 클래스
### 엑스트라 트리
- 랜덤 포레스트와 비슷하게 결정 트리 기반의 앙상블 학습 방법
- 부트스트랩 샘플을 사용하지 않고 랜덤하게 노드를 분할해 과대적합을 감소
